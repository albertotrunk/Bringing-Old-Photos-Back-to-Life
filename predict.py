    ...
    def predict(self, image, HR=False, with_scratch=False):
        try:
            ...
            if not with_scratch:
                ...
                stage_1_command_2 = f"python test.py --Scratch_and_Quality_restore --test_input {new_input} --test_mask {new_mask} --outputs_dir {stage_1_output_dir} --gpu_ids {gpu1}{HR_suffix}"
            ...
            stage_3_input_mask = "./"
            stage_3_command = f"python test_face.py --old_face_folder {stage_3_input_face} --old_face_label_folder {stage_3_input_mask} --tensorboard_log --name {self.opts.checkpoint_name} --gpu_ids {gpu1} --load_size 512 --label_nc 18 --no_instance --preprocess_mode resize --batchSize 1 --results_dir {stage_3_output_dir} --no_parsing_map"
            ...
            stage_4_command = f"python align_warp_back_multiple_dlib_HR.py --origin_url {stage_4_input_image_dir} --replace_url {stage_4_input_face_dir} --save_url {stage_4_output_dir}"
